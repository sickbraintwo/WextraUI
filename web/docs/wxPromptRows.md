...
- **on / off** — the row enters the prompt or not. It also switches a *linked* text: plug an external STRING (another prompt, the trigger words from WLoad Lora & Trigger, a Primitive) into the row's socket on the left edge and turn it on and off from the same chip.
