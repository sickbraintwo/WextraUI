...
Tells you **what you changed since the previous run** of this workflow: seeds, prompts, widget values, rewiring, nodes added or removed. As text, one line per change, right before saving. A week later the file still says what that run changed.
...
- **track**: empty, every node of the workflow is tracked. Write ids here (`74, 12`, or a single widget `74.seed`) and only these are compared: the rest leaves `changes`, `tag`, `count` and the `.log`. The easy way: **select the nodes on the canvas and press `add`** under the box. The label counts what is selected (`add · 3`) and the ids go in, no doubles. **`remove`** takes the selected ones out, with their `74.seed` entries (`remove · 2` = two of the selected are in the box). A node inside a subgraph is written with the id the PROMPT gives it (`12:5`).
- **ignore**: nodes you do not want tracked, by id, separated by commas (`74, 12`); `74.seed` silences one widget of a node you otherwise track. They leave `changes`, `tag`, `count` and the `.log`, even when they are in `track`.
