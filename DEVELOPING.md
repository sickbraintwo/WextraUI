...
the shipped files for them (`check_scanner`); no declared hit today.
...
## 6. The live install is a Registry install

`custom_nodes\WextraUI` is not the repo: since 2026-09-22 it is the package installed from the Comfy Registry through the Manager
(`.tracking` inside, `tools/` and `.github/` are not shipped: see `.comfyignore`). Manager updates come from the Registry, so `main` must
always be publishable. To test a change, copy each changed file over it (`cp` + `cmp`); a Manager update puts the published version back. JS → F5 in the browser; Python → restart
