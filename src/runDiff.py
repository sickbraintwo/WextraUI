...
def parse_ignore(text):
    ...
    `74` = the whole node, `74.seed` = that widget only; a node inside a subgraph has the id the PROMPT gives it (`12:5`).
...
def diff_prompts(old, new, titles, skip_ids=(), records=None, skip_widgets=(), only_ids=(), only_widgets=()):
    ...
    only_ids / only_widgets (`track`): when either is given, only these nodes, or these single widgets, are compared;
    skip still takes away from them."""
    lines = []
    rec = records if records is not None else []
    only_ids, only_widgets = set(only_ids), set(only_widgets)
    tracking = bool(only_ids or only_widgets)
    partly = {p[0] for p in only_widgets} - only_ids   # nodes tracked on some widgets only
    def name(nid):
    ...
    for nid in sorted(set(old) | set(new), key=lambda x: int(x) if x.isdigit() else 0):
        if nid in skip_ids or (tracking and nid not in only_ids and nid not in partly):
            continue
        ...
        for k in sorted(set(oi) | set(ni)):
            ...
            if (nid, k) in skip_widgets or (nid in partly and (nid, k) not in only_widgets):
                continue
...
class RunDiff:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "track": ("STRING", {"default": "", "multiline": True,
                                     "tooltip": "Empty = every node of the workflow is tracked. Or only these, by id, separated by commas: 74, 12 "
                                                "(a single widget: 74.seed): the others leave changes, tag, count and the .log. Select the nodes on "
                                                "the canvas and press add / remove under this box. ignore still takes nodes or widgets out."}),
                "ignore": ("STRING", {"default": "", "multiline": True,
                                      "tooltip": "Nodes not to track, by id, separated by commas: 74, 12. Or a single widget: 74.seed. "
                                                 "They leave changes, tag, count and the .log, even when they are in track."}),
    ...
    def run(self, key="auto", log=True, tag_max=60, changes_max=240, ignore="", passthrough=None, prompt=None, extra_pnginfo=None, unique_id=None, track=""):
        prompt = prompt or {}
        ...
        trk_ids, trk_widgets = parse_ignore(track)
        skip = ign_ids | ({str(unique_id)} if unique_id is not None else set())
        records = []
        lines = diff_prompts(old, prompt, titles, skip, records, ign_widgets, trk_ids, trk_widgets) if old else []
        if not old:
            text, tag = "", "first"
        else:
            text = "\n".join(lines)
