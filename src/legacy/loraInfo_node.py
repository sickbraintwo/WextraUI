...
Helpers (read_header, tag_frequency, civitai_lookup) live in src/loraInfo.py."""
import json
import os
...
class LoraInfo:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "civitai": ("BOOLEAN", {"default": False, "label_on": "look up", "label_off": "file only",
                                        "tooltip": "Also ask Civitai (by SHA-256 of the file) for the official trigger words. "
                                                   "Needs internet the first time; the answer is cached."}),
            },
        }
    ...
    def info(self, lora_name, top_tags=10, civitai=False):
        ...
        if civ:
            rows.append(f"civitai: {civ.get('model')} · {civ.get('name')} · base {civ.get('baseModel')}")
            if civ.get("trainedWords"):
                rows.append("trigger words (civitai): " + ", ".join(civ["trainedWords"]))
        if tags:
            rows.append("top training tags: " + ", ".join(f"{t} ({c})" for t, c in tags[:5]))
        if len(md) <= 3 and not civ:
            rows.append(f"(header has {len(md)} keys: this trainer wrote almost nothing — try civitai)")
