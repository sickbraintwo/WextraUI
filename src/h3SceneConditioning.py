...
class H3SceneConditioning:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "clip": ("CLIP", {"tooltip": "The CLIP used to encode the prompt and the references."}),
                "vae": ("VAE", {"tooltip": "The VAE used to encode frames and keyframes into the latent."}),
                ...
                "ref_image_size": (["match", "max"], {"default": "match",
                                    "tooltip": "match = references scaled (down only) to the generation's "
                                               "pixel area · max = 2048 px short edge, best identity, "
                                               "several times slower."}),
                "picture_order": (PICTURE_ORDER, {"default": "keyframes_first",
                                  "tooltip": "How the text encoder numbers the pictures: keyframes_first = "
                                             "first/last frame come first (the fl2va training order), "
                                             "references after · refs_first = references are numbered first."}),
            },
            "optional": {
    ...
    OUTPUT_TOOLTIPS = (
        "The conditioning for the sampler: first/last frame, guide and references, plus the hand-off when it applies.",
        "The empty latent sized for this scene's canvas and frame count.",
        "This scene's seed, for RandomNoise.",
        "This scene's prompt, as written on WScene H3.",
        "This scene's duration in H3 frames (17k+5 grid).",
        "This scene's duration in seconds, after snapping to the frame grid.",
        "The canvas width actually used for this scene.",
        "The canvas height actually used for this scene.",
        "The frame actually used as the first frame: preview it to check the chain.",
